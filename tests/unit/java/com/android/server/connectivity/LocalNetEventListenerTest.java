...
public class LocalNetEventListenerTest {
    ...
    public void testReportMetrics() throws Exception {
        ...
        LocalNetEventListener listener = new LocalNetEventListener(mDeps, Looper.getMainLooper(),
                true, false);
        listener.start();
        triggerEvent();

        assertTrue("Should have written 2 events",
                eventLatch.await(2000, TimeUnit.MILLISECONDS));
        verify(mDeps).writeStats(1001, 2);
        verify(mDeps).writeStats(4444, 1);
    }

    @Test
    public void testReportMetrics_requiresStart() throws Exception {
        LocalNetEventListener listener = new LocalNetEventListener(mDeps, Looper.getMainLooper(),
                true, false);
    ...
    @Test
    public void testReportMetrics_requiresFlagsEnabled() throws Exception {
        LocalNetEventListener listener = new LocalNetEventListener(mDeps, Looper.getMainLooper(),
                false, false);
        listener.start();
        triggerEvent();
